...
const saveCreated = require('./saveCreated')
...
async function hardLink (input, options) {
  ...
  let { source, saveMode, dest, isDelete, maxFindLevels, exts } = parseCli(input, options)
  ...
  start(source)
  saveCreated(sourceDir, dest, isDelete && !isSecondDir)
}
